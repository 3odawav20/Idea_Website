...
| Account/profile | Account pages | Yes | Read/write own account data | Yes | Supabase profile/session read | Build/static review | CONFIGURATION_REQUIRED | Account identity, RFQ history and quotation history query RLS-protected records; profile editing remains to be wired. |
...
| Shared design tokens | `src/styles/globals.css`, shared UI components | Yes | Common layout, control, focus and motion values come from IDEA variables | No | Implemented | Production build + Chromium | PASS | Shared container and controls now consume tokenized content width and touch-target sizes. |
| Hero image and motion | Home | Yes | Bundled hero imagery remains visible with accessible motion controls | No | Implemented | Chromium 390×844 and 1440×900 | PASS | Both viewports rendered the local hero image; `svh` is used on the mobile hero and reduced-motion remains supported. |
| Product image failure state | Product card/detail/gallery | Yes | A failed documented image URL is visible as unavailable, never replaced by a guessed image | No | Implemented | Chromium product route | PASS | Art Ceramic URLs encountered TLS failure in this environment and rendered explicit fallback states. Source URLs are untouched. |
| Keyboard focus treatment | Global controls | Yes | Keyboard users receive a visible token-based focus indicator | No | Implemented | Build/static review | NEEDS_VERIFICATION | Requires an interactive keyboard traversal sweep across all routes. |
