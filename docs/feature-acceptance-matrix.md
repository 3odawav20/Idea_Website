...
| Favorites | Cards, account | Yes | Persist favorites per authenticated user | Yes | localStorage mock | Code audit | FAIL | Not account-scoped or server-persisted. |
| Buyer registration/login | Auth pages | Yes | Real authenticated identity/session | Yes | localStorage mock | Code audit | FAIL | Client can choose roles. |
...
| RFQ creation | Request quote | Yes | Persist request and route it to suppliers | Yes | localStorage mock | Code audit | FAIL | No database request/recipient records. |
| Supplier quotes | Business, Account | Yes | Persist supplier quotation and calculate totals | Yes | Seed/local simulation | Code audit | FAIL | `seedOffersFor` creates demo quotes/prices. |
| Notifications | Account, Business | Yes | Persist events for correct recipients | Yes | localStorage mock | Code audit | FAIL | No durable recipient security. |
...
| Admin | Admin pages | Yes | Securely manage platform state | Yes | Client-side demo sign-in | Code audit | FAIL | Public UI can create an admin mock session. |
| File uploads | Business/RFQ | Yes | Validate, store, authorize and persist files | Yes | Native local input only | Code audit | FAIL | No Storage bucket, validation, or persistence. |
